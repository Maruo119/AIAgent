...
- **テストコードの作成**: `test_add.py` などのテストコードは、人間が想定される正しい結果になるように事前に修正・作成してください。テストは仕様を定義し、正しい動作を保証します。テストが間違っていると、AI が間違ったコードを生成する可能性があります。
- **AI エージェントの実行**: テストが正しく書かれたら、AI エージェントを実行して対象コード（`target_code.py`）を修正します。AI はテストが通るまでコードを修正します。
- **例**: issue で小数対応が必要な場合、テストに小数のテストケースを追加し、AI がコードを修正します。
